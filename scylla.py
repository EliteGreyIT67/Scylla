...
class Platform(object):
    ...
    def GetOperatingSystemDescriptor(self):
        ...
        if sys.platform == "win32" or sys.platform == "win64":
            ...
            s.print_scylla_notab(
                "OS: Windows | OSINT User: "
                + p.GetUsernameDescriptor()
                + "@"
                + p.GetHostnameDescriptor(),
                color=True,
            )
...
class GEO(object):
    ...
    def geo_api_get_request(self, api_key, ip_address):
        ...
        sender = "https://api.ipgeolocation.io/ipgeo?apiKey={}&ip={}&fields=geo".format(
            str(api_key), ip_address
        )
    ...
    def geo_api_post_request(self, api_key, ip_address):
        ...
        try:
            req = requests.post(
                sender, headers={"User-Agent": random.choice(useragent)}
            )
            jsonRes = req.json()
    ...
    def geo_retrieve_ip_information(self, ip_addr):
        ...
        g = GEO()
        try:
            ip_info = g.geo_api_get_request(
                geolocation_api[0], g.geo_return_ip_address(str(ip_addr))
            )
...
class Web(object):
        ...
        with open("sites.txt", "r") as istr:
            with open("sites_opt.txt", "w") as ostr:
                try:
                    ...
                    s.print_scylla_message(
                        "Searching Known Social Networks For: " + username, color=True
                    )
                    for i, line in enumerate(istr):
                        # Get rid of the trailing newline (if any).
                        line = line.rstrip("\n") + str(username)
                        req = requests.get(
                            line, headers={"User-Agent": random.choice(useragent)}
                        )
                        if req.status_code in success_codes_set:
                            s.print_scylla_valid(
                                line + " => profile FOUND!", color=True
                            )
                        elif req.status_code in error_codes_set:
                            s.print_scylla_invalid(
                                line + " => profile doesn't exist. continuing...",
                                color=True,
                            )
                            continue
    ...
    def scylla_whois(self, domain):
        ...
        with suppress(KeyError):
            info.append(
                "Registered: {}".format(data["creation_date"][0].strftime("%d-%m-%Y"))
            )

        with suppress(KeyError):
            info.append(
                "Expires: {}".format(data["expiration_date"][0].strftime("%d-%m-%Y"))
            )
...
class Shodan(object):
    ...
    def shodan_webcam(self, query):
        ...
        try:
            ...
            s.print_scylla_message(
                "Results Shown May Vary Based On Your API Plan...\n", True
            )
            time.sleep(0.8)
            ...
            for result in results["matches"]:
                ...
                if query == "webcamxp" and port_number == 8080:
                    s.print_scylla_valid(
                        "IP Address/Host-> %s:%s" % (ip_addr, port_number), True
                    )
...
class SMS(object):
    ...
    def ReversePhoneNumberLookup(self):
        s = Scylla()
        api_url = (
            "http://apilayer.net/api/validate?number="
            + self.phone_number
            + "&country_code=&format=1&access_key="
            + str(sms_api[0])
        )
            ...
            for i in array:
                if i in responseObject:
                    s.print_scylla_message(
                        i.replace("_", " ").title()
                        + ": "
                        + str(responseObject[i]).replace("_", " ").title(),
                        color=True,
                    )
...
class Instagram(object):
    ...
    def RetrieveProfileInformation(self):
        web = Web()
        cfg = Config()
        scylla_profile = requests.get(
            str(scyllaUrls[0]) + str(self.username),
        ...
        if scylla_profile.status_code == cfg.SCL_HTTP_GET_SUCCESS:
            ...
            self.bsoup_data = self.t_data["entry_data"]["ProfilePage"][0]["graphql"][
                "user"
            ]
            opt = """
         > Username                :: {}
         ...
         > Profile Picture         :: {}
             """.format(
                str(self.bsoup_data["username"]),
                str(self.bsoup_data["full_name"]),
                str("instagram.com/%s" % self.username),
                str(self.bsoup_data["edge_followed_by"]["count"]),
                str(self.bsoup_data["edge_follow"]["count"]),
                str(self.bsoup_data["edge_owner_to_timeline_media"]["count"]),
                str(self.bsoup_data["biography"]),
                str(self.bsoup_data["external_url"]),
                str(self.bsoup_data["is_private"]),
                str(self.bsoup_data["is_verified"]),
                str(self.bsoup_data["is_business_account"]),
                str(self.bsoup_data["connected_fb_page"]),
                str(self.bsoup_data["is_joined_recently"]),
                str(self.bsoup_data["business_category_name"]),
                web.shorten_url(str(self.bsoup_data["profile_pic_url_hd"])),
            )
...
class Twitter(object):
    ...
    def RetrieveAccountInformation(self):
        ...
        s.print_scylla_message(
            "Gathers Details of " + self.username + " from Twitter", True
        )
        response = None
        try:
        ...
        if response.status_code != 200:
            s.print_scylla_error(
                "ScyllaError", "Invalid Username: " + str(response.status_code), True
            )
            sys.exit(1)
        ...
        try:
            pictureURL = soup.find(
                "a",
                {"class": "ProfileAvatar-container u-block js-tooltip profile-picture"},
            )
        ...
        > Profile Picture URL   :: {}
            """.format(
                ...
                str(data["Followers: "]),
                str(data["Following: "]),
                str(data["Profile URL: "]),
            )
        except KeyError:
            s.print_scylla_error(
                "ScyllaError",
                "@" + self.username + " - Account Cannot Be Scraped",
                True,
            )
...
class Finance(object):
    ...
    def check_card_authenticity(self, card_number):
        cfg = Config()
        if (
            len(str(card_number)) >= cfg.SCL_MIN_CARD_CHAR_LENGTH
            and len(str(card_number)) <= cfg.SCL_MAX_CARD_CHAR_LENGTH
        ):
            return True
        return False
    ...
    def check_breached_card(self, card_number):
        ...
        try:
            if fi.check_card_authenticity(card_number):
                time.sleep(1)
                cprint(
                    "\n\tRetrieving Possible Breach Information...One Moment.",
                    "white",
                    attrs=["bold"],
                )
                ...
                for request in url_queries:
                    for site in search(
                        request, pause=1.3, stop=20, user_agent=random.choice(useragent)
                    ):
                ...
                for element in url_dumps:
                    for breacher in breach_list:
                        if "{}".format(breacher) in element:
                            s.print_scylla_valid(
                                "Card Possibly Pasted @: " + element, True
                            )
    ...
    def finalize_breach_information(self, cardnumber):
        ...
        bin_number_possibility_b = cardnumber[
            0:7
        ]  # For debugging purposes. TODO If 6-digit bin throws exceptions. Try 7
        bin_number_possibility_c = cardnumber[
            0:8
        ]  # For debugging purposes. TODO If 6-digit bin throws exceptions. Try 8
        ...
        if bin_info is not None:
            ...
            s.print_scylla_message(
                "BIN Information Returned(" + bin_number_possibility_a + ")", True
            )

        try:
            fi.check_breached_card(cardnumber)
        except KeyboardInterrupt as ki:
            cprint("Exiting Scylla...", "red")
...
def main():
    ...
    parser.add_argument(
        "-v", "--version", help="returns scyla's version", action="store_true"
    )
    parser.add_argument(
        "-ig",
        "--instagram",
    ...
    if args.instagram:
        ...
        try:
            cprint(
                Instagram(args.instagram).RetrieveProfileInformation(),
                "yellow",
                attrs=["bold"],
            )
        except KeyboardInterrupt as ki:
    ...
    if args.twitter:
        ...
        try:
            cprint(
                Twitter(args.twitter).RetrieveAccountInformation(),
                "green",
                attrs=["bold"],
            )
        except KeyboardInterrupt as ki:
    ...
    if args.username:
        ScyllaBreaker()
        try:
            username_thread = threading.Thread(
                target=w.dump_user_data, args=(args.username,)
            )
    ...
    if args.lookup:
        ScyllaBreaker()
        s.print_scylla_message(
            "Searching Possible Results For: " + args.lookup, color=True
        )
        try:
    ...
    if args.card_info:
        ...
        if " " in args.card_info:
            ...
            fi.finalize_breach_information(args.card_info)

    if args.shodan_query == "webcamxp":
        ScyllaBreaker()
        s.print_scylla_message(
            "Searching ShodanAPI For: " + args.shodan_query, color=True
        )
        try:
            if sh.check_api():
